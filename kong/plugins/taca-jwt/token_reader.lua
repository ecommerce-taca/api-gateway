...
-- Nguồn đọc được trả kèm token: log và metric cần biết token đến bằng đường nào
-- mà không được chạm vào chính giá trị token.
_M.sources = { HEADER = "header", SUBPROTOCOL = "subprotocol", QUERY = "query" }

local function from_authorization_header()
  local value = kong.request.get_header("Authorization")

  return value and string.match(value, BEARER_PATTERN)
end
...
local function from_websocket_subprotocol()
  ...
  if #protocols < 2 or string.lower(protocols[1]) ~= "bearer" then return nil end
...
function _M.read(config)
  ...
  if token then return token, _M.sources.HEADER end

  if config.accept_websocket_subprotocol then
    token = from_websocket_subprotocol()
    if token then return token, _M.sources.SUBPROTOCOL end
  end

  if config.accept_query_token then
    token = from_query_argument()
    if token then return token, _M.sources.QUERY end
  end
