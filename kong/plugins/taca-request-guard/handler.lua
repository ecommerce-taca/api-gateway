...
local function is_trusted_proxy(config)
  if #config.trusted_proxy_cidrs == 0 then
    return false
  end
...
local function serve_readiness(config)
  ...
  if failure_code then
    return envelope_builder.exit(failure_code)
  end

  local body = ops_endpoint.readiness_body(config, checks, envelope_builder.resolve_trace_id())

  return kong.response.exit(200, body, { ["Content-Type"] = JSON_CONTENT_TYPE })
end

function TacaRequestGuardHandler:access(config)
  if config.mode == "readiness" then
    return serve_readiness(config)
  end

  if config.mode == "proxy" then
    return guard_request(config)
  end
end

-- Chế độ liveness và metrics viết lại body của response lấy từ status listener của Kong,
-- nên phải chuẩn bị header trước khi body chảy qua.
function TacaRequestGuardHandler:header_filter(config)
  if config.mode ~= "liveness" and config.mode ~= "metrics" then
    return
  end

  local status = kong.response.get_status()
  if config.mode == "liveness" and status ~= 200 then
